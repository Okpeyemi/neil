...
export default function ChatPage() {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onresult = (e: any) => {
          let interim = '';
            ...
            if (inputRef.current) inputRef.current.placeholder = interim ? '… ' + interim : 'Ask anything / Pose ta question';
        };
  ...
  function toggleRecording() {
    ...
    if (!recording) {
      ...
      setRecording(false);
      if (inputRef.current) inputRef.current.placeholder = 'Ask anything / Pose ta question';
    }
  }
  ...
  async function sendMessage(e?: React.FormEvent) {
    ...
    try {
      ...
      const payloadMessages = messagesRef.current.map(m => ({ role: m.role, content: m.content }));
      ...
      if (data.mode === 'articles_only') {
        ...
        const listMd = arts.length
          ? `Sources trouvées:\n\n${arts.map((a,i)=>`${i+1}. [${a.title}](${a.link})`).join('\n')}`
          : 'Aucune source trouvée.';
        const replyList: Message = { id: crypto.randomUUID(), role: 'assistant', content: '', markdown: listMd };
        setMessages(prev => {
        ...
        setMessages(m => { const updated=[...m, replyMd]; messagesRef.current=updated; return updated; });
        ...
        setMessages(m => { const updated=[...m, replyHtml]; messagesRef.current=updated; return updated; });
        setArticles(data.articles || null);
        setUsedArticles(data.articles || null);
      } else {
        const reply: Message = { id: crypto.randomUUID(), role: 'assistant', content: data.reply || '' };
        setMessages(m => { const updated=[...m, reply]; messagesRef.current=updated; return updated; });
      ...
      setMessages(m => { const updated=[...m, errorMessage]; messagesRef.current=updated; return updated; });
    } finally {
      setLoading(false);
    }
  }
  ...
  <div className="flex-1 px-4 w-full max-w-3xl overflow-auto scroll-hide" ref={listRef}>
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center text-neutral-300/80 select-none gap-6">
              <div>
                <h1 className="text-2xl font-medium mb-4 drop-shadow">Hey, Maqsoud. Ready to dive in?</h1>
                <p className="text-xs opacity-70 mx-auto">Pose une question dans n&apos;importe quelle langue. / Ask in any language.</p>
              </div>
              ...
              <form onSubmit={sendMessage} className="w-full mx-auto">
                <div className="flex items-center gap-2 bg-neutral-900/80 border border-neutral-700 rounded-2xl p-4 backdrop-blur shadow-lg">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      title={speechSupported ? (recording ? 'Arrêter la dictée' : 'Dicter un message') : 'Reconnaissance vocale non supportée'}
                      className={`p-2 transition ${speechSupported ? 'text-neutral-400 hover:text-neutral-200' : 'text-neutral-600 cursor-not-allowed'} ${recording ? 'text-blue-400 mic-recording' : ''}`}
                    ...
                    <button type="submit" disabled={loading || !input.trim()} className="h-9 w-9 flex items-center justify-center rounded-full bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed transition text-white">
                      {loading ? (
                ...
                <p className="text-[10px] text-center mt-3 text-neutral-500">L&apos;IA peut se tromper. / AI may make mistakes.</p>
              </form>
            </div>
          )}
          <div className={`space-y-4 ${messages.length > 0 ? 'pb-40' : ''}`}>
            {messages.map(m => (
              <div key={m.id} className={`flex gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'} message-appear`}>
                {/* Avatar assistant */}
                {m.role === 'assistant' && (
                ...
                {m.markdown ? (
                  <div className={`rounded-2xl px-4 py-3 max-w-[80%] md:max-w-[85%] text-sm shadow-sm bg-neutral-900/70 text-neutral-100 border border-neutral-700`}>
                  ...
                  <div className={`rounded-2xl px-4 py-2 max-w-[80%] md:max-w-[85%] whitespace-pre-wrap leading-relaxed text-sm shadow-sm ${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-100'}`}>{m.content}</div>
                )}
                {m.role === 'user' && (
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 border border-blue-300/40 flex items-center justify-center text-[10px] font-semibold text-white shadow">YOU</div>
                )}
              </div>
            ))}
            {loading && (
        ...
        {messages.length > 0 && (
          <form onSubmit={sendMessage} className="fixed bottom-0 left-0 right-0 w-full bg-gradient-to-t from-black/90 via-black/60 to-transparent">
